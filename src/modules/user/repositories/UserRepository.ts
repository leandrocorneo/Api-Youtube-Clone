            ...
            hash(password, 10, (err, hash) => {
                ...
                'INSERT INTO users (user_id, name, email, password) VALUES (? ,? ,? ,?)',
                    ...
                    (error: any, result: any, fileds: any) =>{
                        ...
                        response.status(200).json({message: 'Usuário criado!'});
                    }
                )
            })
        })
            ...
            connection.query(
                ...
                (error: any, results: any, fileds: any) => {
                    ...
                    compare(password, results[0].password, (err, result) => {
                        if (err) {
                            return response.status(400).json({ error: 'erro na sua autenticaçao!' })
                        }
                        ...
                        if (result){
                            ...
                            return response.status(200).json({token: token, message: 'Autenticação concluída'})
                        }
                    })
                }
            )
